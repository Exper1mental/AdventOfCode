...
for i, line in enumerate(input_file[col_header_row+2:]):
    ...
    col_list[int(entry_list[5])-1] = col_list[int(entry_list[3])-1][0:int(entry_list[1])] + col_list[int(entry_list[5])-1]
    col_list[int(entry_list[3])-1] = col_list[int(entry_list[3])-1][int(entry_list[1]):]

# Output Top Crates in Each Column
top_crates = ''
